...
const fmtC = (v) => {
  // Always returns "Rp X" format, handles negative correctly
  const n = Number(v) || 0
...
// fmtPnL: format P&L dengan tanda + untuk profit
const fmtPnL = (v) => { const n = Number(v)||0; return (n>=0?"+":"")+fmtC(n) }
...
const StatCard = ({ label, value, sub, col, bg, bdr, icon: Icon }) => {
  const { T } = useTheme()
  // Measure value length to auto-scale font size
    ...
    <div style={{ background:bg||T.bg2,border:`1px solid ${bdr||T.bdr}`,borderRadius:18,padding:"14px 12px",minWidth:0,overflow:"hidden" }}>
      ...
      {Icon && (
        <div style={{ marginBottom:7 }}>
          <Icon size={13} color={T.t3} strokeWidth={1.8}/>
        </div>
      )}
      {/* Label */}
      ...
      {/* Sub — small helper text */}
      {sub && (
        <div style={{ fontSize:10,color:T.t2,marginTop:5,fontWeight:500,lineHeight:1.35,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical" }}>
          {sub}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const [editNominal, setEditNominal] = useState("")   // total modal yg diinvestasikan
  const [editMode,    setEditMode]    = useState("avg") // "avg" | "nominal"
...
Avg baru: Rp${newAvg.toFixed(0)} | Alokasi ≤20%. Buka app → tap "Tambah".`,`au_${c}`)
  ...
  const handleBuy = async () => {
    ...
    if (cost>cash) { notify(`Cash tidak cukup! Tersedia: ${fmtC(cash)}`,"red"); return }
  ...
  const handleEdit = async () => {
    ...
    if (!newLot || newLot < 1) { notify("Isi jumlah lot","amber"); return }

    let finalAvg
    if (editMode === "nominal") {
      // User input nominal total → hitung avg otomatis
      const nominal = parsePrice(editNominal)
      if (!nominal || nominal <= 0) { notify("Isi nominal investasi","amber"); return }
      finalAvg = rp(nominal / (newLot * 100))  // avg per lembar
    } else {
      finalAvg = parsePrice(editAvg)
      if (!finalAvg || finalAvg <= 0) { notify("Isi avg price","amber"); return }
    }
    ...
    try {
      ...
      setEditModal(false); setEditPos(null); setEditNominal("")
      notify(`✅ ${editPos.stock_code} — avg Rp ${formatHarga(finalAvg)} · ${newLot} lot`, "green")
    ...
    <ThemeCtx.Provider value={{ T, isDark, toggle:toggleTheme }}>
      ...
      <div style={{ minHeight:"100vh",background:T.bg0,paddingBottom:100,maxWidth:500,margin:"0 auto" }}>
        ...
        {tab==="home" && (() => {
          ...
          const menuTiles = [
            { id:"forecast",  Ic:TrendingUp,   label:"Forecast",  desc:"Prediksi & sinyal" },
            { id:"calendar",  Ic:CalendarIcon, label:"Kalender",  desc:"Dividen & event" },
            { id:"strategi",  Ic:Lightbulb,    label:"Strategi",  desc:"Simulasi 20 thn" },
          ]
          ...
          return (
            <div style={{ maxWidth:480,margin:"0 auto",padding:"16px 16px 120px" }}>
              ...
              <div style={{ background:T.bg1,border:`1px solid ${T.bdr2}`,borderRadius:22,padding:"20px",marginBottom:16,position:"relative",overflow:"hidden" }}>
                {/* Top accent bar */}
                <div style={{ position:"absolute",top:0,left:0,right:0,height:3,borderRadius:"22px 22px 0 0",background:isUp_?T.green:T.red }}/>
                {/* Label row */}
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6 }}>
                  <span style={{ fontSize:10,fontWeight:800,letterSpacing:"1.5px",color:T.t3 }}>TOTAL EKUITAS</span>
                  <span style={{ fontSize:10,fontWeight:700,color:isUp_?T.green:T.red,background:isUp_?T.gBg:T.rBg,padding:"2px 8px",borderRadius:99 }}>
                    {isUp_?"↑ PROFIT":"↓ LOSS"}
                  </span>
                </div>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
                  <div style={{ background:isUp_?T.gBg:T.rBg,border:`1px solid ${isUp_?T.gBdr:T.rBdr}`,borderRadius:99,padding:"5px 12px",display:"inline-flex",alignItems:"center",gap:6 }}>
                    <span style={{ fontSize:13,fontWeight:800,color:isUp_?T.green:T.red }}>
                      {fmtPnL(unrealPnL_)}
                    </span>
                    <span style={{ fontSize:11,color:T.t3 }}>|</span>
                    <span style={{ fontSize:12,fontWeight:700,color:isUp_?T.green:T.red }}>
                      {unrealPct_.toFixed(2)}%
                    </span>
                  </div>
                  <span style={{ fontSize:10,color:T.t3 }}>Unrealized P&L</span>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div style={{ display:"inline-flex",alignItems:"center",gap:6,marginTop:12,background:unrealPnL>=0?T.gBg:T.rBg,border:`1px solid ${unrealPnL>=0?T.gBdr:T.rBdr}`,padding:"6px 14px",borderRadius:99 }}>
                    ...
                    <span style={{ fontSize:13,fontWeight:800,color:unrealPnL>=0?T.green:T.red }}>
                      {fmtPnL(unrealPnL)} ({formatPercent(capital>0?(unrealPnL/capital)*100:0)})
                    </span>
                  </div>
                </div>
              ...
              <div style={{ background:T.bg2,border:`1px solid ${T.bdr}`,borderRadius:18,padding:16 }}>
                <div style={{ display:"flex",justifyContent:"space-between",marginBottom:10 }}>
                  ...
                  <span style={{ fontSize:14,fontWeight:900,color:{red:T.red,amber:T.amber,green:T.green}[cashStatus] }}>
                    {cashPct.toFixed(1)}% <span style={{ fontSize:11,fontWeight:600,color:T.t3 }}>| {fmtC(cash)}</span>
                  </span>
                </div>
                ...
                return (
                  <div key={pos.id} className="fu" style={{ background:T.bg1,border:`1px solid ${isSL?T.rBdr:over20?T.aBdr:T.bdr2}`,borderRadius:18,overflow:"hidden",marginBottom:12,animationDelay:`${pidx*0.04}s` }}>
                    ...
                    <div style={{ padding:"14px 14px 0" }}>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
                        ...
                        <div style={{ textAlign:"right" }}>
                          ...
                          <div style={{ fontSize:10,fontWeight:700,color:pnlPct>=0?T.green:T.red }}>{fmtPnL(pnlRp)}</div>
                        </div>
                      </div>
                        ...
                        <div style={{ marginBottom:12 }}>
                          <div style={{ background:T.lBg,border:`1px solid ${T.lBdr}`,borderRadius:10,padding:"10px 12px",marginBottom:7 }}>
                            ...
                            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px 10px",fontSize:10 }}>
                              {[["Maks beli",canLot+" lot ("+fmtC(canRp)+")"],["Avg baru","Rp "+formatHarga(newAvg)],["Alokasi baru",newAlloc.toFixed(1)+"%"],["SL baru","Rp "+formatHarga(newAvg*0.92)]].map(([l,v])=>(
                    ...
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",borderTop:`1px solid ${T.bdr}` }}>
                      ...
                      <button onClick={()=>{ setEditPos(pos); setEditAvg(String(pos.avg_price)); setEditLot(String(pos.lot)); setEditNominal(""); setEditMode("avg"); setEditModal(true) }} style={{ background:"transparent",border:"none",borderRight:`1px solid ${T.bdr}`,padding:"11px",fontSize:12,fontWeight:800,color:T.t2,cursor:"pointer" }} className="tap">Edit</button>
                    ...
                    <div style={{ background:T.bg2,padding:"12px 16px",borderRadius:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom: hasNote||isSell?10:0 }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        ...
                        <div style={{ fontSize:15,fontWeight:900,color:isSell?getProfitColor(row.pnl,T):isTopup?T.blue:T.t1 }}>
                          {isSell ? fmtPnL(row.pnl||0) : `${fmtC(row.pos_val||(row.lot*100*row.close_price))}`}
                        </div>
                      </div>
                    </div>
          ...
          <div className="fu" style={{ padding:"56px 20px 20px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,gap:12 }}>
              <div style={{ minWidth:0,flex:1 }}>
                <h2 style={{ fontSize:24,fontWeight:900,color:T.t1,marginBottom:3,letterSpacing:"-0.5px" }}>Analyst Monitor</h2>
                <p style={{ fontSize:12,color:T.t3,lineHeight:1.4 }}>Performa & kesehatan portofolio real-time.</p>
              </div>
              <div style={{ flexShrink:0 }}><ThemeBtn/></div>
            </div>
            ...
            <div style={{ background:T.bg1,border:`1px solid ${T.bdr2}`,borderRadius:20,padding:20,marginBottom:16 }}>
              ...
              {[
                ...
                <div key={lbl} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.bdr}`,gap:8,minWidth:0 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8,minWidth:0,flex:1 }}>
                    <div style={{ width:8,height:8,borderRadius:"50%",background:col,flexShrink:0 }}/>
                    <span style={{ fontSize:12,color:T.t2,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{lbl}</span>
                  </div>
                  <span style={{ fontSize:13,fontWeight:800,color:col,whiteSpace:"nowrap",flexShrink:0 }}>
                    {fmtC(val)}
                  </span>
                </div>
              ))}
            ...
            <div style={{ background:T.bg1,border:`1px solid ${T.bdr2}`,borderRadius:20,padding:20 }}>
              <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:20 }}>
                <PieChart size={15} color={T.t3} strokeWidth={1.6}/>
                    ...
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        ...
                        <div style={{ fontSize:11,fontWeight:700,color:getProfitColor(posPnl,T) }}>
                          {fmtPnL(posPnl)} ({formatPercent(posPnlPct)})
                        </div>
                      </div>
                    </div>
          ...
          <div className="fu" style={{ padding:"56px 20px 100px" }}>
            ...
            <div style={{ background:T.bg1,border:`1px solid ${T.bdr2}`,borderRadius:20,padding:20,marginBottom:20 }}>
              ...
              <div style={{ fontSize:12,color:T.t2,marginBottom:16 }}>
                Modal {fmtC(Math.max(capital,1_000_000))} × return {targetReturn}%/tahun (dengan variasi pasar wajar)
              </div>

              {/* Summary */}
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16 }}>
                {[
                  ["Nilai Akhir", fmtC(finalBal), T.green],
                  ["Total Profit", fmtC(totalProfit), T.blue],
                  ...
                  {simRows.map(r => {
                    ...
                    return (
                      <div key={r.y} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}>
                        <div title={`Tahun ${r.y}: ${fmtC(r.bal)} (${r.ret}%)`}
                ...
                {[5,10,15,20].map(y => {
                  ...
                  return (
                    <div key={y} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:T.bg2,borderRadius:10,marginBottom:6 }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontSize:14,fontWeight:900,color:T.green }}>{fmtC(r.bal)}</div>
                        <div style={{ fontSize:10,color:T.t3 }}>+{fmtC(r.bal - simRows[0].prev)}</div>
                      </div>
                    </div>
                  )
                })}
        ...
        <Modal open={addModal} onClose={()=>setAddModal(false)} title={`Beli ${addStock?.c||addStock?.stock_code||""}`} subtitle="Buka posisi baru atau Average Down">
          <div style={{ background:T.bg2,padding:16,borderRadius:16,marginBottom:20,display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
            ...
            <div><div style={{ fontSize:10,fontWeight:800,color:T.t3,marginBottom:4 }}>CASH TERSEDIA</div><div style={{ fontSize:15,fontWeight:800,color:T.em }}>{fmtC(cash)}</div></div>
        ...
        {/* ══ EDIT POSISI MODAL ══ */}
        <Modal open={editModal} onClose={()=>{ setEditModal(false); setEditPos(null); setEditNominal(""); setEditMode("avg") }} title={`Sinkronisasi Posisi ${editPos?.stock_code}`} subtitle="Samakan dengan data di aplikasi sekuritas kamu">

          {/* ── Data saat ini ── */}
          <div style={{ background:T.bg2,borderRadius:14,padding:"12px 16px",marginBottom:16,display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10 }}>
            <div>
              <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:4,letterSpacing:"0.5px" }}>AVG SEKARANG</div>
              <div style={{ fontSize:14,fontWeight:800,color:T.t1 }}>Rp {formatHarga(editPos?.avg_price)}</div>
            </div>
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:4,letterSpacing:"0.5px" }}>LOT</div>
              <div style={{ fontSize:14,fontWeight:800,color:T.t1 }}>{editPos?.lot}</div>
            </div>
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:4,letterSpacing:"0.5px" }}>TOTAL MODAL</div>
              <div style={{ fontSize:14,fontWeight:800,color:T.em }}>
                Rp {new Intl.NumberFormat("id-ID",{notation:"compact",maximumFractionDigits:1}).format((editPos?.lot||0)*100*(editPos?.avg_price||0))}
              </div>
            </div>
          </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:18 }}>
            {[
              { id:"avg",     label:"Input Harga Avg",    sub:"Masukkan avg per lembar" },
              { id:"nominal", label:"Input Total Modal",  sub:"Masukkan nominal Rupiah" },
            ].map(m => (
              <button key={m.id} onClick={()=>setEditMode(m.id)}
                style={{ padding:"10px 12px",borderRadius:12,border:`2px solid ${editMode===m.id?T.em:T.bdr2}`,
                  background:editMode===m.id?T.emBg:T.bg2,cursor:"pointer",textAlign:"left",transition:"all .15s" }}>
                <div style={{ fontSize:12,fontWeight:800,color:editMode===m.id?T.em:T.t1,marginBottom:2 }}>{m.label}</div>
                <div style={{ fontSize:10,color:T.t3 }}>{m.sub}</div>
              </button>
            ))}
          </div>

          {/* ── Input lot (selalu ada) ── */}
          <Input label="JUMLAH LOT SEKARANG" type="number" hint="Sesuaikan dengan di aplikasi sekuritas" value={editLot} onChange={e=>setEditLot(e.target.value)}/>

          {/* ── Input sesuai mode ── */}
          {editMode === "avg" ? (
            <Input label="HARGA AVERAGE (Rp/lembar)" type="number" prefix="Rp"
              hint="Lihat di kolom 'Avg Price' atau 'HPP' di sekuritas kamu"
              value={editAvg} onChange={e=>setEditAvg(e.target.value)}/>
          ) : (
            <Input label="TOTAL MODAL DIINVESTASIKAN (Rp)" type="number" prefix="Rp"
              hint="Lihat kolom 'Nilai Beli' atau 'Modal' di sekuritas kamu — sistem hitung avg otomatis"
              value={editNominal} onChange={e=>setEditNominal(e.target.value)}/>
          )}

          {/* ── Preview hasil ── */}
          {(() => {
            const lot_  = parseInt(editLot) || 0
            const shares_ = lot_ * 100
            let previewAvg = 0, previewNominal = 0, valid = false

            if (editMode === "avg") {
              previewAvg     = parsePrice(editAvg)
              previewNominal = rp(shares_ * previewAvg)
              valid = previewAvg > 0 && lot_ > 0
            } else {
              previewNominal = parsePrice(editNominal)
              previewAvg     = shares_ > 0 ? rp(previewNominal / shares_) : 0
              valid = previewNominal > 0 && lot_ > 0
            }

            const live_    = liveCache[editPos?.stock_code]?.price || editPos?.avg_price || 0
            const curVal_  = shares_ * live_
            const pnlRp_   = rp(curVal_ - previewNominal)
            const pnlPct_  = previewNominal > 0 ? rp((pnlRp_ / previewNominal) * 100) : 0
            const isProfit = pnlRp_ >= 0

            if (!valid) return null
            return (
              <div style={{ background:T.bg2,border:`1px solid ${T.bdr2}`,borderRadius:14,padding:14,marginBottom:16 }}>
                <div style={{ fontSize:10,fontWeight:800,color:T.t3,marginBottom:10,letterSpacing:"0.5px" }}>PREVIEW SETELAH DISIMPAN</div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10 }}>
                  <div style={{ background:T.bg3,borderRadius:10,padding:"10px 12px" }}>
                    <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:4 }}>AVG PRICE BARU</div>
                    <div style={{ fontSize:16,fontWeight:900,color:T.em }}>Rp {formatHarga(previewAvg)}</div>
                    {editMode==="nominal" && <div style={{ fontSize:10,color:T.t3,marginTop:2 }}>dihitung otomatis</div>}
                  </div>
                  <div style={{ background:T.bg3,borderRadius:10,padding:"10px 12px" }}>
                    <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:4 }}>TOTAL MODAL</div>
                    <div style={{ fontSize:16,fontWeight:900,color:T.t1 }}>
                      Rp {new Intl.NumberFormat("id-ID",{notation:"compact",maximumFractionDigits:1}).format(previewNominal)}
                    </div>
                    {editMode==="avg" && <div style={{ fontSize:10,color:T.t3,marginTop:2 }}>{lot_} lot × 100 × {formatHarga(previewAvg)}</div>}
                  </div>
                </div>
                {live_ > 0 && (
                  <div style={{ background:isProfit?T.gBg:T.rBg,border:`1px solid ${isProfit?T.gBdr:T.rBdr}`,borderRadius:10,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                    <div>
                      <div style={{ fontSize:9,fontWeight:800,color:T.t3,marginBottom:2 }}>P&L VS HARGA LIVE</div>
                      <div style={{ fontSize:11,color:T.t3 }}>Live Rp {formatHarga(live_)}</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:16,fontWeight:900,color:isProfit?T.green:T.red }}>
                        {isProfit?"+":""}{pnlPct_.toFixed(2)}%
                      </div>
                      <div style={{ fontSize:11,color:isProfit?T.green:T.red }}>
                        {isProfit?"+":""}{pnlRp_ >= 0 ? "" : "-"}Rp {Math.abs(pnlRp_).toLocaleString("id-ID",{maximumFractionDigits:0})}
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div style={{ background:T.aBg,border:`1px solid ${T.aBdr}`,borderRadius:12,padding:"10px 14px",marginBottom:16 }}>
            <div style={{ fontSize:11,color:T.amber,lineHeight:1.5 }}>
              ℹ️ Perubahan ini <strong>tidak mencatat transaksi baru</strong> di jurnal. Hanya menyinkronkan data porto dengan sekuritas kamu.
            </div>
          </div>

          <Btn full onClick={handleEdit}>💾 Simpan & Sinkronkan</Btn>
        </Modal>
